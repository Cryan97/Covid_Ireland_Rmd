...
transformed parameters{
  vector[T] lambda; 
    for (t in 1:T){
      lambda[t] = ((theta_1*theta_3*exp(-theta_3*(t-theta_2)))/(1+exp(-theta_3*(t-theta_2)))^2);
    }
}
model {
...
generated quantities {
  ...
  for (i in 1:5){
    cases_pred[i] = poisson_rng(((theta_1*theta_3*exp(-theta_3*((T+i)-theta_2)))/(1+exp(-theta_3*((T+i)-theta_2)))^2));
  }
}